...
import static ntu.csie.keydial.Symbols.*;
...
public class Prompter extends Application {
	...
	enum State {
		Connecting, SelectKeyboard, CharacterTraining, PhraseInput, Complete;
	}
	...
	private Cursor<String> character;
	private Cursor<String> phrase;

	public void start() throws Exception {
		keyboard = new Cursor<>(args.getKeyboardOrder());
		character = new Cursor<>(args.getCharacterTrainingSequence());
	...
	public boolean onRecord(Record r) {
		...
		case CharacterTraining:
			if (Record.LETTER.matcher(r.key).matches()) {
				promptNextCharacter();
				return true;
			} else if (ENTER.equals(r.key)) {
				promptNextPhrase();
				return true;
			}

			// collect bad keystrokes
			setText(status, "IGNORE KEY: " + r.key);
			return true;
		case PhraseInput:
	...
	public void promptNextCharacter() {
		setState(State.CharacterTraining);

		if (character.hasNext()) {
			setText(prompt, character.next());
			setText(progress, String.format("%d / %d", character.position(), character.length()));
			TTS.sayLetter(character.current());
			return;
		}

		say("Character Training complete.");
		setText(prompt, "Press ENTER to continue.");
	}
